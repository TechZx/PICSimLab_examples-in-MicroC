#line 1 "C:/Users/ZX/Desktop/Final 01323/part1/part1.c"
 char *mem_data[10];
char *mem2_data[10];
char *mem3_data [10] ;
char minutes[2];
char hours[2];
char temph;
char tempm;
char day[2];
char month[2];
char year[4];
char tempDay;
char tempMonth;
char tempYear;

char read_keypad()
{
 char var, key;

 PORTD = 0xFF;
 TRISD = 0xFF;

 TRISB = 0x00;
 PORTB = 0xFF;

 key =0;
 do {

 PORTB.F0 = 0;
 delay_ms(10);
 var = PORTD;

 if (var.F2 == 0) key = '4';
 else if (var.F3 == 0) key = '1';
 PORTB.F0 =1;


 PORTB.F1 = 0;
 delay_ms(10);
 var = PORTD;

 if (var.F2 == 0)key = '5';
 else if (var.F3 == 0)key = '2';
 PORTB.F1 =1;


 PORTB.F2 =0;
 delay_ms(10);
 var = PORTD;

 if (var.F3 == 0) key = '3';
 PORTB.F2 =1;
 }while (key == 0);

 return key;
}

unsigned char function_numbers(unsigned char v)
{
 switch(v)
 {
 case 1:
 return 0x06;
 case 2:
 return 0x5B;
 case 3:
 return 0x4F;
 case 4:
 return 0x66;
 case 5:
 return 0x6D;

 }
 return 0;
}


void main() {

 char keypressed;
 int i=0;


 keypressed = read_keypad();


 TRISD = 0x00;
 TRISA = 0xFB;


 keypressed = keypressed - 0x30;

 PORTD = function_numbers(keypressed);
 switch (function_numbers(keypressed))
 {
 case 0x06:

 TRISE = 0x00;
 TRISD = 0x00;
 I2C_Init(100000);


 Soft_I2C_Config(&PORTC, 4, 3);
 Soft_I2C_Start();
 Soft_I2C_Write(0xD0);
 Soft_I2C_Write(1);
 Soft_I2C_Start();
 Soft_I2C_Write(0xD1);
 tempm = Soft_I2C_Read(0);
 Soft_I2C_Stop();
 minutes[0]=((tempm & 0x70)>>4)+0x30;
 minutes[1]= (tempm & 0x0F) +0x30;


 Soft_I2C_Config(&PORTC, 4, 3);
 Soft_I2C_Start();
 Soft_I2C_Write(0xD0);
 Soft_I2C_Write(2);
 Soft_I2C_Start();
 Soft_I2C_Write(0xD1);
 temph = Soft_I2C_Read(0);
 Soft_I2C_Stop();
 hours[0]=((temph & 0x30)>>4)+0x30;
 hours[1]= (temph & 0x0F) +0x30;



 Soft_I2C_Config(&PORTC, 4, 3);
 Soft_I2C_Start();
 Soft_I2C_Write(0xA2);
 Soft_I2C_Write(2);
 Soft_I2C_Write(hours);
 Soft_I2C_Write(minutes);
 Soft_I2C_Stop();

 Soft_I2C_Start();
 Soft_I2C_Write(0xA2);
 Soft_I2C_Write(2);
 Soft_I2C_Start();
 Soft_I2C_Write(0xA3);
 mem_data[0] = Soft_I2C_Read(1);
 mem_data[1] = Soft_I2C_Read(1);



 Soft_I2C_Config(&PORTC, 4, 3);
 Soft_I2C_Start();
 Soft_I2C_Write(0xD0);
 Soft_I2C_Write(4);
 Soft_I2C_Start();
 Soft_I2C_Write(0xD1);
 tempDay = Soft_I2C_Read(0);
 Soft_I2C_Stop();
 day[0]=((tempDay & 0x70)>>4)+0x30;
 day[1]= (tempDay & 0x0F) +0x30;


 Soft_I2C_Config(&PORTC, 4, 3);
 Soft_I2C_Start();
 Soft_I2C_Write(0xD0);
 Soft_I2C_Write(5);
 Soft_I2C_Start();
 Soft_I2C_Write(0xD1);
 tempMonth = Soft_I2C_Read(0);
 Soft_I2C_Stop();
 month[0]=((tempMonth & 0x30)>>4)+0x30;
 month[1]= (tempMonth & 0x0F) +0x30;


 Soft_I2C_Config(&PORTC, 4, 3);
 Soft_I2C_Start();
 Soft_I2C_Write(0xD0);
 Soft_I2C_Write(6);
 Soft_I2C_Start();
 Soft_I2C_Write(0xD1);
 tempYear = Soft_I2C_Read(0);
 Soft_I2C_Stop();
 year[0]=((tempYear & 0x70)>>4)+0x30;
 year[1]= (tempYear & 0x0F) +0x30;




 Soft_I2C_Config(&PORTC, 4, 3);
 Soft_I2C_Start();
 Soft_I2C_Write(0xA2);
 Soft_I2C_Write(2);
 Soft_I2C_Write(day);

 Soft_I2C_Write(month);
 Soft_I2C_Write(year);

 Soft_I2C_Stop();

 Soft_I2C_Start();
 Soft_I2C_Write(0xA2);
 Soft_I2C_Write(2);
 Soft_I2C_Start();
 Soft_I2C_Write(0xA3);
 mem2_data[0] = Soft_I2C_Read(1);
 mem2_data[1] = Soft_I2C_Read(1);
 mem2_data[2] = Soft_I2C_Read(1);




 Lcd8_Config(&PORTE, &PORTD, 2,1,0, 7,6,5,4,3,2,1,0);
 Lcd8_Cmd(LCD_CURSOR_OFF);
 Lcd8_Cmd(LCD_CLEAR);


 Lcd8_Out(1, 1,"Time=");
 Lcd8_Out(1, 6,mem_data[0]);
 Lcd8_Out(1, 8,":") ;
 Lcd8_Out(1, 9,mem_data[1]);

 Lcd8_Out(1, 11," ") ;
 Lcd8_Out(1, 12," ") ;
 Lcd8_Out(1, 13," ") ;
 Lcd8_Out(1, 14," ") ;
 Lcd8_Out(1, 15," ") ;
 Lcd8_Out(1, 16," ") ;

 Lcd8_Out(2, 1,"Date=");
 Lcd8_Out(2, 6,mem2_data[0]);
 Lcd8_Out(2, 8,"/") ;
 Lcd8_Out(2, 9,mem2_data[1]);
 Lcd8_Out(2, 11,"/") ;
 Lcd8_Out(2, 12,mem2_data[2]);
 Lcd8_Out(2, 14,mem2_data[2]);
 }
}
